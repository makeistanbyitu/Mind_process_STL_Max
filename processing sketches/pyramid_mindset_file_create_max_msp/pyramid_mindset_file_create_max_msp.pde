...
void setup() {
  ...
  wallThickness = 7;
  gridSize = 3;
  mindSet = new MindSet(this, "COM8"); //needs to be set to whichever COM port the mindest is communicating to
...
void build() {
  ...
  for (int x = 0; x < gridSize; x++) {
    for (int y = 0; y < gridSize; y++) {
      ...
      float[] peakAngle = {delta_mapped, theta_mapped, low_alpha_mapped, high_alpha_mapped, low_beta_mapped, high_beta_mapped, low_gamma_mapped, mid_gamma_mapped, attention_};//random(25, 65); // steepness of the pyramd
...
delta_mapped=(map(float(delta), 3000,3600000,0,1000));
theta_mapped=(map(float(theta), 2000,1700000,0,1000));
...
mid_gamma_mapped=(map(float(mid_gamma), 1000,2100000,0,1000));
